...
import { DEFAULT_REWARDS_PRESETS } from "./rewards-config";
import {
  sampleUsers,
  sampleArtworks,
  sampleHubs,
...
export const createInMemoryBackend = (provider: "firebase" | "supabase"): BackendAdapter => {
  ...
  return {
    ...
    rewards: {
      ...
      presets: async () => DEFAULT_REWARDS_PRESETS.quickActions,
      updatePresets: async (input) => {
        Object.assign(DEFAULT_REWARDS_PRESETS.quickActions, input);
        return DEFAULT_REWARDS_PRESETS.quickActions;
      }
    },
    uploads: {
