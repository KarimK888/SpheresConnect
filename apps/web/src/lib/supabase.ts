...
import { DEFAULT_REWARDS_PRESETS, type QuickRewardAction } from "./rewards-config";
import {
  sampleHubs,
  sampleArtworks,
  sampleOrders,
...
let fallbackRewardPresets: Record<QuickRewardAction, number> = { ...DEFAULT_REWARDS_PRESETS.quickActions };
...
let supportsRewardNoteColumn = true;
...
export const mapUserRow = (row: Database["public"]["Tables"]["users"]["Row"]): User => ({
  userId: row.user_id,
  email: row.email,
...
const isMissingRewardNoteColumnError = (error: unknown): error is PostgrestError =>
  Boolean(
    error &&
      typeof error === "object" &&
      "code" in error &&
      ...
      (error as PostgrestError).message.toLowerCase().includes("'note' column")
  );
  ...
  return {
    ...
    auth: {
      ...
      oauth: async ({ provider, token }) => {
        ...
        const result = await anon.auth.signInWithIdToken({
          provider: targetProvider,
          token
        });
        if (result.error || !result.data.user || !result.data.session) {
          throw result.error ?? new Error("Unable to complete OAuth sign-in");
        }
        const { data, error } = await admin.from("users").select("*").eq("user_id", result.data.user.id).single();
        if (error && error.code !== "PGRST116") {
          throw error;
        }
        let userRow = data;
        if (!userRow) {
          const profileRow: Database["public"]["Tables"]["users"]["Insert"] = {
            ...
            bio: null,
            skills: [],
            profile_picture_url: (result.data.user.user_metadata?.avatar_url as string | undefined) ?? null,
            connections: [],
            is_verified: false,
            role: "member",
            language: "en",
            location: null,
            joined_at: result.data.user.created_at ?? new Date().toISOString(),
            profile: null
          };
          const upserted = await admin.from("users").upsert(profileRow, { onConflict: "user_id" }).select().single();
          if (upserted.error || !upserted.data) {
            throw upserted.error ?? new Error("Unable to persist profile");
          }
          userRow = upserted.data;
        }
      ...
      resetPassword: async ({ password }) => {
        ...
        const { data: updateData, error: updateError } = await anon.auth.updateUser({ password });
        if (updateError || !updateData.user) {
          throw updateError ?? new Error("Unable to reset password");
        }
        const { data: row } = await admin.from("users").select("*").eq("user_id", updateData.user.id).single();
        if (!row) {
          currentSession = null;
          return null;
        }
    ...
    users: {
      list: async ({ query }) => {
        ...
        if (query) {
          ...
          builder.or(`display_name.ilike.%${query}%,email.ilike.%${query}%,skills.cs.{${normalized}}`);
        }
        const { data, error } = await builder;
        if (error) {
          console.warn("[supabase] users.list falling back to sample data:", error.message);
          return query ? [] : sampleUsers;
        }
    ...
    rewards: {
      ...
      log: async (log) => {
        ...
        try {
          const { admin } = clients;
          const buildPayload = (includeNote: boolean) => ({
            id: entry.id,
            user_id: entry.userId,
            action: entry.action,
            points: entry.points,
            created_at: new Date(entry.createdAt).toISOString(),
            ...(includeNote ? { note: entry.note ?? null } : {})
          });
          let { error } = await admin.from("rewards").insert(buildPayload(supportsRewardNoteColumn));
          if (error && supportsRewardNoteColumn && isMissingRewardNoteColumnError(error)) {
            supportsRewardNoteColumn = false;
            ({ error } = await admin.from("rewards").insert(buildPayload(false)));
          }
          if (error) throw error;
          fallbackRewards = [...fallbackRewards, entry];
          return entry;
      ...
      presets: async () => {
        if (!clients) {
          return fallbackRewardPresets;
        }
        try {
          const { admin } = clients;
          const { data, error } = await admin.from("rewards_presets").select("action, points");
          if (error || !data) {
            throw error ?? new Error("Unable to load reward presets");
          }
          const mapped: Record<QuickRewardAction, number> = { ...fallbackRewardPresets };
          data.forEach((row) => {
            const key = row?.action as QuickRewardAction | undefined;
            if (key && typeof row.points === "number") {
              mapped[key] = row.points;
            }
          });
          fallbackRewardPresets = mapped;
          return mapped;
        } catch (error) {
          console.warn("[supabase] rewards.presets falling back:", error);
          return fallbackRewardPresets;
        }
      },
      updatePresets: async (input) => {
        if (!clients) {
          fallbackRewardPresets = { ...fallbackRewardPresets, ...input };
          return fallbackRewardPresets;
        }
        try {
          const { admin } = clients;
          const rows = Object.entries(input).map(([action, points]) => ({ action, points }));
          const { error } = await admin.from("rewards_presets").upsert(rows, { onConflict: "action" });
          if (error) throw error;
          fallbackRewardPresets = { ...fallbackRewardPresets, ...input };
          return fallbackRewardPresets;
        } catch (error) {
          console.warn("[supabase] rewards.updatePresets falling back:", error);
          fallbackRewardPresets = { ...fallbackRewardPresets, ...input };
          return fallbackRewardPresets;
        }
      }
