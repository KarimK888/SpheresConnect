...
export const MarketplaceCheckoutClient = ({ artwork, sellerName }: MarketplaceCheckoutClientProps) => {
  ...
  const startCheckout = async () => {
    ...
    try {
      ...
      if (offline) {
        ...
        setStatusMessage(t("checkout_offline_queue"));
        return;
      }
      ...
      if (!response.ok) {
        ...
        throw new Error(errorPayload?.error?.message ?? t("checkout_error_generic"));
      }
      const intentPayload = (await response.json()) as CheckoutIntent;
  ...
  const confirmPayment = async () => {
    ...
    try {
      const offline = typeof navigator !== "undefined" && navigator.onLine === false;
      const payload = { paymentIntentId: intent.order.stripePaymentIntentId, status: "paid" };
      if (offline) {
        await enqueueMutation({
        ...
        setStatusMessage(t("checkout_confirm_offline"));
        return;
      }
  ...
  return (
    <div className="space-y-8">
      <div className={cn("flex flex-col gap-4", isMobile && "text-center gap-3")}>
        <button
          type="button"
          className={cn(
            "inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-white",
      ...
      <div className={cn("grid gap-6 lg:grid-cols-[2fr,1fr]", isMobile && "grid-cols-1")}>
        ...
        <Card className={cn("border-border/60 bg-card/80", !isMobile ? "sticky top-6 self-start" : "order-2")}>
          <CardHeader>
