                    ...
                    <div className="grid gap-3 md:grid-cols-2">
                      <div>
                        ...
                        <select
                          ...
                          onChange={(event) =>
                            setEditForm((prev) => ({ ...prev, shippingStatus: event.target.value }))
                          }
                        >
                          {["processing", "preparing", "in_transit", "delivered", "refunded"].map((value) => (
                            <option key={value} value={value} className="bg-[#050816] text-black">
                              {t(`marketplace_orders_shipping_${value}`)}
                            </option>
                          ))}
                        </select>
                      </div>
...
const buildTimeline = (order: Order, shippingStatus: string, t: TranslateFn) => {
  const createdDetail = new Date(order.createdAt).toLocaleDateString();
  const paymentComplete = order.status === "paid";
  const fulfillmentDetail = t(`marketplace_orders_shipping_${shippingStatus}`);
  return [
    {
      key: "created",
